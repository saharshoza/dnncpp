...
*/
		...
		case opType::e_minus:{
			...
			else if(v1.cols() == 1){value = v0.colwise() + v1.col(0); }
		...
		case opType::e_colmax:{
			value = parents[0]->value.rowwise().maxCoeff();
			//value = parents[0]->value.colwise() - maxVal.col(0);
			return;
		}
		case opType::e_softmax:{
			MatrixXf maxVal = parents[0]->value.rowwise().maxCoeff();
			MatrixXf correctedVal = parents[0]->value.colwise() - maxVal.col(0);
			MatrixXf expVal = correctedVal.array().exp().matrix();
			MatrixXf rowSum = expVal.rowwise().sum();
			MatrixXf logVal = rowSum.array().log().matrix();
			MatrixXf logCorrectedVal = logVal.colwise() + maxVal.col(0);
			value = parents[0]->value.colwise() - logCorrectedVal.col(0);
			value = -1 * value;
/*
			std::cout << "maxval " << maxVal << std::endl;
			std::cout << "correctedVal " << correctedVal << std::endl;
			std::cout << "expVal " << expVal << std::endl;
			std::cout << "rowSum " << rowSum << std::endl;
			std::cout << "logVal " << logVal << std::endl;
			std::cout << "logCorrectedVal " << logCorrectedVal << std::endl;
			std::cout << "parent " << parents[0]->value << std::endl;
...
bool Node<MatrixXf>:: areChildrenDone(){
	...
	for(auto n: children){
		if(n->gradientParent.find(this) == n->gradientParent.end()){ 
			...
			else{	MatrixXf gradParent = n->gradientParent[this];
				gradient += gradParent; }
		}
...
//	std::cout << "col2imImage flattenGradient " << flattenGradient << std::endl;
...
//	std::cout << "col2imImage input " << input << std::endl;
...
void Node<Tensor<float, 4>>::back(){
	switch(op){
		...
		case opType::e_conv:{
			...
			//Eigen::ThreadPool pool(16);
			//Eigen::ThreadPoolDevice currDevice(&pool, 16);

			Tensor<float, 3> inputFilterAll(kernelSize*kernelSize*depth, 1, numFilters);	
			for(int f = 0; f < numFilters; ++f){
				...
				for(int depth = 0; depth < inputDim[2]; ++depth){
					for(int i = 0; i < kernelSize; ++i){
						for(int j = 0; j < kernelSize; ++j){
							inputFilterAll(i*kernelSize + j + depth*kernelSize*kernelSize, 0, f) = currKernel(i, j, depth, 0);
						}
					}
				}
			...
			for(int n = 0; n < numExamples; ++n){ 
				...
				// Flattened Gradient across all filters
				Tensor<float, 3> flattenGradient(((height-kernelSize)/1 + 1)*((width-kernelSize)/1 + 1), 1, numFilters);
				...
				inputGradientAccumulate.setZero();
				...
				for(int f = 0; f < numFilters; ++f){
					...
					Tensor<float, 2> flattenGradientCurrKernel(((height-kernelSize)/1 + 1)*((width-kernelSize)/1 + 1), 1);

					for(int col = 0; col < ((width-kernelSize)/1 + 1); ++col){
						for(int row = 0; row < ((height-kernelSize)/1 + 1); ++row){
							flattenGradientCurrKernel(row*((height-kernelSize)/1 + 1) + col, 0) = slicedGradient(row, col, 0, 0);
							flattenGradient(row*((height-kernelSize)/1 + 1) + col, 0, f) = slicedGradient(row, col, 0, 0);
						}
					}

					// Kernel slicing bounds	
					array<long, 3> kernelStart = {0, 0, f};
					array<long, 3> kernelExtent = {kernelSize*kernelSize*depth, 1, 1};
					array<long, 2> reshapeDim({kernelSize*kernelSize*depth, 1});
					Tensor<float, 2> inputFilter = inputFilterAll.slice(kernelStart, kernelExtent).reshape(reshapeDim);

					array<long, 2> shuffle = {1, 0};
					Tensor<float, 2> inputFilterTranspose = inputFilter.shuffle(shuffle);
					Eigen::array<Eigen::IndexPair<int>, 1> product_dims = { Eigen::IndexPair<int>(1, 0) };
					//Tensor<float, 2> inputGradient(625, 16);
					//inputGradient.device(currDevice) = flattenGradientCurrKernel.contract(inputFilterTranspose, product_dims);
					Tensor<float, 2> inputGradient = flattenGradientCurrKernel.contract(inputFilterTranspose, product_dims);
				...
				Eigen::array<Eigen::IndexPair<int>, 1> product_dims = { Eigen::IndexPair<int>(1, 0) };
				Tensor<float, 3> filterGradient = inputMat.contract(flattenGradient, product_dims);	
				Eigen::array<long, 4> kernelReshapeDim({kernelDim[0], kernelDim[1], kernelDim[2], kernelDim[3]});
				Tensor<float, 4> filterGradientIter(kernelDim[0], kernelDim[1], kernelDim[2], kernelDim[3]);
				filterGradientIter = filterGradient.reshape(kernelReshapeDim);
				filterParentGradient += filterGradientIter;
...
void Node<MatrixXf>::back(){
	switch(op){
		...
		case opType::e_colmax:{
			gradientParent[parents[0]] = MatrixXf::Zero(parents[0]->value.rows(), parents[0]->value.cols());
			MatrixXf::Index maxIndex;
			std::cout << gradient << std::endl;
			for(int i = 0; i < parents[0]->value.rows(); ++i){
				parents[0]->value.row(i).maxCoeff(&maxIndex);
				gradientParent[parents[0]](i, maxIndex) = gradient(i, maxIndex);
			}
			std::cout << gradientParent[parents[0]] << std::endl;
			return;
		}
		case opType::e_sum:{
...
*/
		...
		case opType::e_log:{	
			int gradCols = gradient.cols();
			int parentCols = parents[0]->value.cols();
			if(gradCols == parentCols){
				gradientParent[parents[0]] = gradient.cwiseProduct(parents[0]->value.cwiseInverse());
			}
			else { gradientParent[parents[0]] = gradient.rowwise().sum().cwiseProduct(parents[0]->value.cwiseInverse());
			}
			return;
		}
		case opType::e_colsum:{	
		...
		case opType::e_softmax:{
			MatrixXf logGradient = gradient.cwiseProduct(parents[0]->value.cwiseInverse());
			MatrixXf colSumGradient = MatrixXf::Ones(logGradient.rows(), logGradient.cols());
...
void Node<MatrixXf>::update(){
	...
	if(!variable) {return;}
	int gradRows = gradient.rows();
	int gradCols = gradient.cols();
	int Rows = value.rows();
	
	float learningRate = 1e-2;

	std::cout << "Gradient " << learningRate*gradient << std::endl;
	// SGD
/*
	if(gradRows != Rows && Rows == 1){ 
			value -= learningRate*gradient.colwise().sum(); }
	else { value -= learningRate*gradient; }
*/

	// Adagrad
	if(gradRows != Rows && Rows == 1){
		if(gradCache.size() == 0) {gradCache = MatrixXf::Zero(1, gradient.cols()); }
		gradCache += gradient.colwise().sum().array().square().matrix();
		float eps = 1e-6;
		MatrixXf epsMatrix = MatrixXf::Ones(gradCache.rows(), gradCache.cols());
		epsMatrix = eps * epsMatrix;
		MatrixXf gradDenom = gradCache.array().sqrt().matrix() + epsMatrix;	
		MatrixXf gradUpdate = gradient.colwise().sum().cwiseQuotient(gradDenom);
		value -= learningRate*gradUpdate;
	}
	else{
		if(gradCache.size() == 0) {gradCache = MatrixXf::Zero(gradient.rows(), gradient.cols()); }
		gradCache += gradient.array().square().matrix();
		float eps = 1e-6;
		MatrixXf epsMatrix = MatrixXf::Ones(gradCache.rows(), gradCache.cols());
		epsMatrix = eps * epsMatrix;
		MatrixXf gradDenom = gradCache.array().sqrt().matrix() + epsMatrix;	
		MatrixXf gradUpdate = gradient.cwiseQuotient(gradDenom);

		value = value - learningRate*gradUpdate;
	}
...
void Node<Tensor<float, 4>>::update(){
	...
	// SGD
	//value = value - learningRate*gradient;
		
	// Adagrad
	if(gradCache.size() == 0) {
		Tensor<float, 4> zeroTensor(gradient.dimensions());
		zeroTensor.setZero();
		gradCache = zeroTensor; 
	}
	gradCache += gradient.square();
	float eps = 1e-6;
	Tensor<float, 4> gradDenom = gradCache.sqrt() + eps;	
	Tensor<float, 4> gradUpdate = gradient/gradDenom;
